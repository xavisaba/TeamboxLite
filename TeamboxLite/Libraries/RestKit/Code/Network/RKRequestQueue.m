...
// BLOOSEE--------------------------------------------
@synthesize requestsList = _requests;
//----------------------------------------------------
...
- (void)loadNextInQueue {
	...
	if (self.suspended) {
        ...
        NSLog(@"Deferring request loading for queue %@ due to suspension", self);
		return;
	}
...
- (BOOL)removeRequest:(RKRequest*)request decrementCounter:(BOOL)decrementCounter {
    ...
    //BlooSee//RKLogWarning(@"Failed to remove request %@ from queue %@: it is not in the queue.", request, self);
    RKLogDebug(@"Failed to remove request %@ from queue %@: it is not in the queue.", request, self);
    return NO;
}
